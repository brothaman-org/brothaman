...
Let's first set up the pgAdmin web application running in a container using a Podman quadlet. The pgAdmin web application will run in its own unprivileged user account called `pgadmin`. The PostgreSQL server will run in its own unprivileged user account called `postgresql`. Both containers will run on the same host but isolated from each other in separate unprivileged users accounts. pgAdmin will connect to the PostgreSQL server over its systemd socket port on `localhost:5432`. Log into the virtual machine as the `vagrant` user and issue the following commands:
