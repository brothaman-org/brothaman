...
After=idp-private-network.service
Requires=idp-private-network.service
...
Now after putting the first container into the new idp-private network and activating it you should be able to ping the gateway and see the network interfaces from within the unprivileged user's podman unshare rootless network namespace. Try this as the `vagrant` user:
