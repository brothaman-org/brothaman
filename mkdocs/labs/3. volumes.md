...
Now let's see how we can do all these things EZ PZ with Brothaman:

```bash
sudo bro-user --remove postgres
sudo bro-user postgres
sudo -iu postgres podman run --rm quay.io/podman/hello
sudo -iu postgres mkdir -p ~/.config/containers/systemd/
cat <<'EOF' > ~/.config/containers/systemd/postgresql.container
[Unit]
Description=PostgreSQL Container
...
Environment=POSTGRES_PASSWORD=password
Volume=/var/lib/containers/unprivileged/postgres/volumes/postgresql:/var/lib/postgresql/data
UserNS=keep-id
