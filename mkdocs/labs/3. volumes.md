...
Systemd is extremely powerful and an amazing init system. Learning to use its features effectively can greatly enhance your ability to manage services and their dependencies. Unfortunately, we snuck these in, with only slight mentions as describe here to limit the scope of this lab.
...
With `zfs-helper` in place, we updated the volume quadlet to use it for snapshot creation and deletion. It removed a lot of the pain we experienced first hand. We also implemented automatic snapshot cleanup in the simplest way to maintain a manageable number of snapshots, a primitive snapshot retention policy manager. Finally, we connected the volume quadlet to the PostgreSQL container quadlet and tested data persistence and snapshot rollbacks, demonstrating the effectiveness and power of ZFS snapshots and rollbacks for data protection and recovery.

Hopefully by now you've started to understand why a separate volume quadlet is a good thing. It allows you to manage the lifecycle of persistent storage independently from the container, share volumes between multiple containers, and leverage advanced features like ZFS snapshots for data protection and recovery. Although we could also do this inside the container quadlet directly, separating concerns into dedicated quadlets leads to cleaner, more maintainable configurations.

Let's end with an exercise. If we put everything into the container quadlet, failures due to snapshot management would be mixed with failures due to the container and to the postgresql service mixing the two separate concerns and making it much harder to troubleshoot. Conversely with systemd dependency management we are able to explicitly see problems arising from the volume verses the container. As an exercise, try putting everything into the container quadlet and see how it complicates things. Then revert back to the separated volume quadlet approach to see the benefits clearly.
